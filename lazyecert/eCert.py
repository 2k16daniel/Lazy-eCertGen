...
def excel_parser(excel_file):
    ...
    nrow_max = len([row for row in sheet if not all([cell.value == None for cell in row])])
    for i in range(2,nrow_max):
        Name.append(sheet.cell(row = i, column= 3).value+" "+sheet.cell(row = i, column= 4).value+" "+sheet.cell(row = i, column= 5).value)
    return Name